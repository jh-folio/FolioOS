...
Folio OS 0.4.3은 내 PC에 저장한 시장 뉴스와 리서치 자료를 모아 일일 브리핑, 시장 맥락, 기업 분석, 근거 추적이 가능한 Deep Research로 정리해 주는 로컬 투자 리서치 도구입니다.
...
- Research Cockpit 대시보드에서 보고서 생성 시 확인된 변화, 종목·지수 차트, 시장 캘린더, Portfolio 연결 영향을 한 화면에서 점검하기 (Cockpit/Legacy 전환 버튼으로 기존 위젯 보드도 계속 쓸 수 있음)
- 로컬 워치리스트에서 기업·섹터·테마를 추적하고 저지연 lead와 보고서 기반 Change Feed를 분리해 보기
- FinancialJuice, 사용 권한이 있는 Investing.com RSS, Benzinga RSS, 기존 한국 RSS에서 빠르게 게시된 제목·링크만 모은 빠른 시장 신호 확인하기 (본문은 저장하지 않으며, 확인 전 정보는 보고서 근거나 중대한 변화 판정에 넣지 않음)
...
0.4.3 사용자 화면에 포함하지 않은 기능:

- 설치 프로그램/트레이 앱 수준의 마감 작업

저지연 뉴스는 검증된 근거가 아니라 초기 lead입니다. FinancialJuice 무료 스트림은 약 10분 지연될 수 있습니다. Investing.com과 Benzinga는 제공자가 허용한 RSS URL이 있을 때만 수집하며 페이지 스크래핑이나 유료 본문 우회를 하지 않습니다. 외부 Vision은 기본 비활성이고 crop 이미지마다 별도 동의가 필요합니다. 이미지 원본, raw OCR 텍스트, bounding box는 import 작업이 끝난 뒤 보관하지 않습니다.
...
현재 시장 위젯, 최근 보고서, 확인할 항목, 그리고 보유 종목의 요약을 한 화면에서 점검합니다. 시장 위젯은 TradingView를 사용하며 출처 표기를 유지합니다.
