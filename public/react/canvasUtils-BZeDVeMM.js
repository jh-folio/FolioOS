import { L as e, _ as t, p as n, v as r, z as i } from "./Geometry-DGpI5VLa.js";
...
a.extension = i.TextureSource;
//#endregion
//#region node_modules/pixi.js/lib/rendering/renderers/canvas/utils/canUseNewCanvasBlendModes.mjs
var o;
function s(e) {
	let t = n.get().createCanvas(6, 1), r = t.getContext("2d");
	return r.fillStyle = e, r.fillRect(0, 0, 6, 1), t;
}
function c() {
	if (o !== void 0) return o;
	try {
		let e = s("#ff00ff"), t = s("#ffff00"), r = n.get().createCanvas(6, 1).getContext("2d");
		r.globalCompositeOperation = "multiply", r.drawImage(e, 0, 0), r.drawImage(t, 2, 0);
		let i = r.getImageData(2, 0, 1, 1);
		if (!i) o = !1;
		else {
			let e = i.data;
			o = e[0] === 255 && e[1] === 0 && e[2] === 0;
		}
	} catch {
		o = !1;
	}
...
var l = {
	...
	getCanvasSource: (e) => {
		...
		if (!r) return null;
		let i = t.alphaMode === "premultiplied-alpha", a = t.resourceWidth ?? t.pixelWidth, o = t.resourceHeight ?? t.pixelHeight, s = a !== t.pixelWidth || o !== t.pixelHeight;
		if (i) {
			if ((r instanceof HTMLCanvasElement || typeof OffscreenCanvas < "u" && r instanceof OffscreenCanvas) && !s) return r;
			let e = l._unpremultipliedCache.get(t);
			if (e?.resourceId === t._resourceId) return e.canvas;
		}
		if (r instanceof Uint8Array || r instanceof Uint8ClampedArray || r instanceof Int8Array || r instanceof Uint16Array || r instanceof Int16Array || r instanceof Uint32Array || r instanceof Int32Array || r instanceof Float32Array || r instanceof ArrayBuffer) {
			let e = l._canvasSourceCache.get(t);
			if (e?.resourceId === t._resourceId) return e.canvas;
			let i = n.get().createCanvas(t.pixelWidth, t.pixelHeight), a = i.getContext("2d"), o = a.createImageData(t.pixelWidth, t.pixelHeight), s = o.data, c = r instanceof ArrayBuffer ? new Uint8Array(r) : new Uint8Array(r.buffer, r.byteOffset, r.byteLength);
			if (t.format === "bgra8unorm") for (let e = 0; e < s.length && e + 3 < c.length; e += 4) s[e] = c[e + 2], s[e + 1] = c[e + 1], s[e + 2] = c[e], s[e + 3] = c[e + 3];
			else s.set(c.subarray(0, s.length));
			return a.putImageData(o, 0, 0), l._canvasSourceCache.set(t, {
				canvas: i,
				resourceId: t._resourceId
			}), i;
		}
		if (i) {
			let e = n.get().createCanvas(t.pixelWidth, t.pixelHeight), i = e.getContext("2d", { willReadFrequently: !0 });
			e.width = t.pixelWidth, e.height = t.pixelHeight, i.drawImage(r, 0, 0);
			let a = i.getImageData(0, 0, e.width, e.height), o = a.data;
			for (let e = 0; e < o.length; e += 4) {
				let t = o[e + 3];
				if (t > 0) {
					let n = 255 / t;
					o[e] = Math.min(255, o[e] * n + .5), o[e + 1] = Math.min(255, o[e + 1] * n + .5), o[e + 2] = Math.min(255, o[e + 2] * n + .5);
				}
			}
			return i.putImageData(a, 0, 0), l._unpremultipliedCache.set(t, {
				canvas: e,
				resourceId: t._resourceId
			}), e;
		}
		if (s) {
			let e = l._canvasSourceCache.get(t);
			if (e?.resourceId === t._resourceId) return e.canvas;
			let i = n.get().createCanvas(t.pixelWidth, t.pixelHeight), a = i.getContext("2d");
			return i.width = t.pixelWidth, i.height = t.pixelHeight, a.drawImage(r, 0, 0), l._canvasSourceCache.set(t, {
				canvas: i,
				resourceId: t._resourceId
			}), i;
		}
	...
	getTintedCanvas: (t, r) => {
		let i = t.texture, a = e.shared.setValue(r).toHex(), o = i.tintCache ||= {}, s = o[a], c = i.source._resourceId;
		if (s?.tintId === c) return s;
		let u = s && "getContext" in s ? s : n.get().createCanvas();
		if (l.tintMethod(i, r, u), u.tintId = c, l.convertTintToImage && u.toDataURL !== void 0) {
			let e = n.get().createImage();
			e.src = u.toDataURL(), e.tintId = c, o[a] = e;
		} else o[a] = u;
		return o[a];
	},
	getTintedPattern: (t, r) => {
		let i = e.shared.setValue(r).toHex(), a = t.patternCache ||= {}, o = t.source._resourceId, s = a[i];
		return s?.tintId === o ? s : (l.canvas ||= n.get().createCanvas(), l.tintMethod(t, r, l.canvas), s = l.canvas.getContext("2d").createPattern(l.canvas, "repeat"), s.tintId = o, a[i] = s, s);
	},
