import { M as e, O as t, R as n, l as r, n as i, r as a } from "./Geometry-DGpI5VLa.js";
import { s as o } from "./RenderTargetSystem-DxXuAFdm.js";
import { p as s } from "./GCManagedHash-DosM8O0d.js";
...
function p(e, n) {
	for (let r in e.attributes) {
		let i = e.attributes[r], a = n[r];
		a ? (i.format ??= a.format, i.offset ??= a.offset, i.instance ??= a.instance) : t(`Attribute ${r} is not present in the shader, but is present in the geometry. Unable to infer attribute details.`);
	}
	m(e);
}
function m(e) {
	let { buffers: t, attributes: n } = e, i = {}, a = {};
	for (let e in t) {
		let n = t[e];
		i[n.uid] = 0, a[n.uid] = 0;
	}
	for (let e in n) {
		let t = n[e];
		i[t.buffer.uid] += r(t.format).stride;
	}
	for (let e in n) {
		let t = n[e];
		t.stride ??= i[t.buffer.uid], t.start ??= a[t.buffer.uid], a[t.buffer.uid] += r(t.format).stride;
	}
}
//#endregion
...
var b = {
	...
	constructor({ buffer: t, offset: n, size: r }) {
		super(), this.uid = e("buffer"), this._resourceType = "bufferResource", this._touched = 0, this._resourceId = e("resource"), this._bufferResource = !0, this.destroyed = !1, this.buffer = t, this.offset = n | 0, this.size = r, this.buffer.on("change", this.onBufferChange, this);
	}
	onBufferChange() {
