...
- **완료 단계의 튜토리얼은 선택이지 절차가 아니다.** 진행 막대의 단계로 세지 않는다 — 세면 건너뛸 수 없는 것처럼 읽힌다. 자료 수집 → 시장 내러티브 → 브리핑·보고서 흐름 3장이며 화면 사용법이 아니다.
...
- **사전작업(`runPrerequisites`)도 예약별이다.** 브리핑 전에 RSS 수집과 시장 메모리 갱신을 돌릴지이며, 값이 없으면 켠 것으로 읽는다. 0.5.2에서 화면 토글이 사라지고 값만 `true`로 박혀 있었다 — RSS 자동 수집을 끈 구성에서는 이것이 유일한 수집 경로다.
