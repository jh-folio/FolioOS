...
export function ConsultationPanel() {
  ...
  return (
    <aside ref={panelRef} className="consultation-panel" aria-label="투자 상담" aria-modal="true" role="dialog">
      ...
      {legacyNotice && <div className="settings-notice warn"><strong>기존 로컬 대화가 있습니다.</strong><span>자동으로 옮기거나 삭제하지 않습니다. 필요한 내용을 확인해 새 상담에 직접 붙여 넣을 수 있습니다.</span><button className="btn" type="button" onClick={() => { dismissLegacyConsultationNotice(); setLegacyNotice(false); }}>확인</button></div>}
      <div className="consultation-layout">
        <nav className="consultation-sessions" aria-label="저장된 상담"><button className="btn btn--primary" type="button" onClick={() => createSession(pendingOpen)}>새 상담</button>{sessions.map((row) => <button type="button" className={row.id === session?.id ? "active" : ""} onClick={() => loadSession(row.id)} key={row.id}><strong>{row.title}</strong><small>{row.scope.kind} · {row.messageCount || 0} turns</small></button>)}</nav>
        <section className="consultation-thread">
          {session ? <>
            <div className="consultation-toolbar"><input aria-label="상담 제목" value={titleDraft} onChange={(event) => setTitleDraft(event.currentTarget.value)} /><button className="btn" type="button" onClick={rename}>이름 저장</button><button className="btn" type="button" onClick={exportLocal}>내보내기</button><button className="btn" type="button" onClick={archive}>보관</button><button className="btn" type="button" onClick={remove}>삭제</button></div>
            ...
            {waitingMessage && <button className="btn" type="button" disabled={busy} onClick={retryWaiting}>저장된 질문 다시 답변</button>}
            <form className="consultation-composer" onSubmit={submit}><textarea value={input} onChange={(event) => setInput(event.currentTarget.value)} placeholder="지금 궁금한 점이나 검토할 상황을 그대로 적어보세요." rows={4} /><div><button className="btn" type="button" onClick={previewNote}>노트로 정리</button><button className="btn btn--primary" type="submit" disabled={busy || !input.trim()}>{busy ? "답변 작성 중" : "보내기"}</button></div></form>
          </> : <p className="cockpit-empty">상담을 선택하거나 새로 시작하세요.</p>}
          {error && <p className="react-dashboard-error" role="alert">{error}</p>}
        </section>
      </div>
      {noteDraft && <div className="consultation-note-preview"><h3>노트 미리보기</h3><label>유형<select value={noteDraft.noteType} onChange={(event) => setNoteDraft({ ...noteDraft, noteType: event.currentTarget.value as NoteDraft["noteType"] })}><option value="company_thesis">기업 thesis</option><option value="portfolio_decision">Portfolio 판단</option><option value="investment_note">투자 노트</option></select></label><label>제목<input value={noteDraft.title} onChange={(event) => setNoteDraft({ ...noteDraft, title: event.currentTarget.value })} /></label><label>본문<textarea rows={10} value={noteDraft.body} onChange={(event) => setNoteDraft({ ...noteDraft, body: event.currentTarget.value })} /></label><div><button className="btn" type="button" onClick={() => setNoteDraft(null)}>취소</button><button className="btn btn--primary" type="button" onClick={saveNote}>내 생각 노트로 저장</button></div></div>}
    </aside>
  );
}
