...
describe("Classic asset manifest", () => {
  it("accepts one shell, all semantic scene objects, a neutral fallback, and one character atlas", () => {
    ...
    expect(sceneObjects).toHaveLength(13);
  ...
  it("draws every pulled-out chair in front of its own workstation", () => {
    const depthOf = (id: string) => {
      const asset = CLASSIC_ASSET_MANIFEST.assets.find((item) => item.id === id) as { depth?: number } | undefined;
      if (!asset || asset.depth === undefined) throw new Error(`missing depth for ${id}`);
      return asset.depth;
    };
    // 의자가 책상보다 앞(화면 아래)에 있으므로 항상 나중에 그려져야 한다.
    // 이 순서가 뒤집히면 의자가 책상에 먹혀 "가구가 겹쳤다"로 보인다.
    for (const [workstation, chair] of [
      ["classic_news_desk_workstation_object", "classic_news_desk_chair_object"],
      ["classic_research_desk_workstation_object", "classic_research_desk_chair_object"],
      ["classic_portfolio_workstation_object", "classic_portfolio_chair_object"],
    ]) {
      expect(depthOf(chair)).toBeGreaterThan(depthOf(workstation));
    }
    // 러그는 바닥 장식이라 라운지 가구 전부보다 먼저 그려져야 한다.
    const rug = depthOf("classic_agent_seat_rug_object");
    for (const id of [
      "classic_agent_seat_chair_object",
      "classic_agent_seat_side_table_object",
      "classic_agent_seat_coffee_table_object",
    ]) {
      expect(rug).toBeLessThan(depthOf(id));
    }
  });
...
describe("Authored route graph", () => {
  it("is strongly connected and passes through authored hubs", () => {
    ...
    // 경로 모양은 배치에 따라 달라지지만, 오브젝트끼리 직접 잇지 않고
    // 항상 열린 바닥의 허브를 거쳐야 한다는 계약은 배치와 무관하게 유지된다.
    expect(route.nodes[0]).toBe("news_desk");
    expect(route.nodes[route.nodes.length - 1]).toBe("portfolio_monitor");
    expect(route.nodes.length).toBeGreaterThanOrEqual(3);
    for (const node of route.nodes.slice(1, -1)) {
      expect(["room_hub", "north_hub", "south_hub"]).toContain(node);
    }
  });
});
