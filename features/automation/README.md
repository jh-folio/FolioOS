...
- 스냅샷 생성은 `_refresh_market_state_snapshot()`이며 버튼과 **같은 경로**를 쓴다: CLI 모드는 `run_market_memory_update_task`(중기 메모리 → 스냅샷 2단계, `job_id` 없이 부르면 동기), API 모드는 `run_llm_market_state_snapshot`. 따로 만들면 버튼과 예약이 서로 다른 스냅샷을 만들게 된다.
