...
| 출처 | 기본 상태 | 비고 |
|---|---|---|
| FinancialJuice 공개 RSS (`/feed.ashx?xy=rss`) | **켜짐** | 키 불필요, 제목만 제공되어 metadata-only 계약과 일치 |
| FinancialJuice Stream API | 키 있을 때만 | `FINANCIALJUICE_API_KEY` 저장 시 WebSocket으로 자동 승격(무료 10분 지연 표시). 키 없음은 정상 상태이며 실패로 기록하지 않음 |
...
매일경제는 fast-origin이 아니라 일반 RSS로 분류합니다. Investing.com은 공개 피드 주소가 없고 약관상 사전 승인이 필요해 0.4.x 범위에서 제외했습니다. Wall St Engine, First Squawk, TradingView 뉴스, scraping, 제3자 RSS 변환기도 범위가 아닙니다.
