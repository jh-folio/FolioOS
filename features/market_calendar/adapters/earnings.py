...
def estimated_earnings_events(tickers: list[str]) -> list[dict]:
    ...
    for ticker in tickers[:30]:
        try:
            ...
            events.append(normalize_event({"kind": "earnings", "title": f"{ticker} 실적 발표 예정", "tickers": [ticker], "startsAt": starts, "timezone": timezone, "status": "estimated", "source": "yfinance", "provider": "yfinance", "market": market, "companyName": company_name(ticker), "importance": 2}))
        except Exception:
            continue
    return events
