...
`GET /api/market-calendar`의 `ticker` 파라미터는 서버에서 거른다. `tickers_json`이 리스트라 티커 판정을 파이썬에서 하므로 **티커를 물었을 때는 SQL LIMIT을 걸지 않고 필터 뒤에 절단한다** — 먼저 자르면 티커와 무관한 앞쪽 일정이 LIMIT을 소진해 종목 일정이 한 건도 남지 않는다(실측: 90일 창의 앞 20건이 전부 macro라 `ticker=AVGO&limit=20`이 항상 빈 결과였다).
...
- **정리 범위는 종류별로 나눠 넘긴다**(`{"earnings": {...}, "dividend": {...}}`). 실적은 `Ticker.calendar`, 배당은 `get_info()`로 서로 다른 호출이라 한쪽만 rate limit에 걸리는 일이 흔한데, 두 집합을 합치면 물어보지도 못한 종류의 멀쩡한 행이 "답에 없다"로 지워진다.
