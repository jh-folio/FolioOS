...
def _recent(hours: int = 1) -> str:
    """지금 기준 상대 시각. 고정 날짜를 쓰면 신선도 규칙 때문에 테스트가 시간이 지나며 깨진다."""
...
def test_source_failure_is_isolated_to_its_objects():
    loaders = _loaders(index={"count": 4, "generatedAt": _recent()})
...
def test_job_source_failure_marks_only_agent_seat_as_error():
    loaders = _loaders(index={"count": 1, "generatedAt": _recent()})
...
def test_default_index_summary_reads_sqlite_without_building(tmp_path):
    ...
    generated_at = _recent()
    (data_dir / "index.json").write_text(
        json.dumps({"count": 99, "generatedAt": generated_at}),
        encoding="utf-8",
    )
...
def test_default_note_summary_never_reads_note_body(tmp_path):
    ...
    assert rows == [{"id": "note-1", "updatedAt": updated_at}]
