...
- 오늘 또는 선택 날짜의 브리핑 생성
- 과거 브리핑 조회
- 통합 생성 시 시장 간 연결 분석(`{date}.link.json`)을 별도 레이어로 생성
...
통합 생성은 각 시장 Canonical 본문을 바꾸지 않는 **별도 레이어**로 `{date}.link.json`을 만듭니다. 읽기 시에만 통합 뷰 본문 위에 얹습니다.

시장이 넷이면 쌍은 여섯입니다. 여섯 쌍을 모두 훑는 보고서는 분석이 아니라 소음이므로, 기본 서술은 **세션이 실제로 마감한 순서**를 따르고 쌍 단위 메타데이터는 **동인을 공유하는 쌍만** 남깁니다.

세션 순서는 고정 목록이 아니라 각 시장 세션일의 한국시간 마감 시각으로 정렬합니다.

| 시장 | 한국시간 마감(근사) |
| --- | --- |
| 유럽 | 세션일 익일 00:30 (런던 16:30) |
| 미국 | 세션일 익일 05:00 (뉴욕 16:00 ET) |
| 일본·한국 | 세션일 당일 15:30 |

일본·한국이 개장 전이면 그 브리핑은 **전일** 세션을 다루고, 그 세션은 간밤의 유럽·미국보다 **먼저** 닫혔습니다. 고정 목록으로 정렬하면 시간을 거슬러 올라가는 전달 경로를 서술하게 됩니다.

- **전달 경로**는 체인에서 인접한 세션 사이에만 만듭니다. 연속으로 닫힌 두 세션이 동인을 공유하는 것이, 서로 이어지지 않은 두 세션이 공유하는 것보다 전달을 주장할 근거가 됩니다.
- **글로벌 채널**(금리·환율/달러·원자재/유가)은 두 시장 이상에 동시에 잡힐 때만 기록합니다.
- 어느 시장과도 동인을 공유하지 않는 시장은 `decoupledMarkets`로 이름을 밝힙니다. "연결이 약하다"가 아니라 "이 자료에서는 따로 움직였다"는 관찰입니다.
- 이슈가 하나도 없는 시장이 있으면 전체 상태는 `independent`가 아니라 `insufficient_evidence`입니다. 근거의 부재는 탈동조화의 근거가 아닙니다.
- 시장 하나가 빠져도 분석은 만들어집니다. 세 시장도 아무것도 없는 것보다는 설명합니다. 빠진 시장은 `limitations`에 남습니다.

저장된 `both` 보고서 독자는 `usOnlyDrivers`/`krOnlyDrivers`와 `## 한미 시장 연결 분석` 제목을 읽으므로, 두 시장 경로(`build_link_analysis`)는 그 계약을 그대로 유지합니다. 네 시장 경로는 `build_cross_market_analysis`이고 제목은 `## 시장 간 연결 요약`입니다. LLM 심화는 초안 모양에 맞는 프롬프트를 골라 쓰며, 실패하면 규칙 기반 본문이 그대로 남습니다.
